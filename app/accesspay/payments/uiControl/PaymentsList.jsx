...
// ════════════════════════════════════════════════════════════════
// Imports from completed-payments.jsx
import {
  filterCompletedPayments
} from '../logicControl/completed-payments';

// Imports from recoveries.jsx
import {
  filterRecoveries
} from '../logicControl/recoveries';

// Imports from disbursements.jsx
import {
  filterDisbursements
} from '../logicControl/disbursements';
              ...
              <Fragment key={`_row_${listpayments_result.primkey}`}>
                <tr key={listpayments_result.primkey}>
                  <td>
                    <div className="table_cell_dropdown">
                      <div className="table_cell_dropbtn">
                        ...
                        <div className="table_cell_dropdown-content">
                          ...
                          icon="list "
                          dataIn={() => viewRequests({childCol:`recordId`,parentColVal:listpayments_result.request_id,parentName:listpayments_result.reference_number})}   // only runs on click now
                          callBack={(incomingRequest) => {setChildDataOut(incomingRequest) }}
                          />
                          <MosyGridRowOptions
                          src="PaymentsList"
                          ...
                          label=" Staff Details"
                          icon="list "
                          dataIn={() => viewStaff({childCol:`recordId`,parentColVal:listpayments_result.staff_id,parentName:listpayments_result.reference_number})}   // only runs on click now
                          callBack={(incomingRequest) => {setChildDataOut(incomingRequest) }}
                          />
                        </div>
                      </div>
                    ...
                    <td scope="col"><span title={listpayments_result.request_id}>{magicTrimText(listpayments_result._requests_request_number_request_id, 70)}</span></td>
                    <td scope="col"><span title={listpayments_result.staff_id}>{magicTrimText(listpayments_result._staff_full_name_staff_id, 70)}</span></td>
                    <td scope="col"><span title={listpayments_result.payment_type}>{magicTrimText(listpayments_result.payment_type, 70)}</span></td>
                    <td scope="col"><span>{mosyTonum(listpayments_result.amount)}</span></td>
                    <td scope="col"><span title={listpayments_result.reference_number}>{magicTrimText(listpayments_result.reference_number, 70)}</span></td>
