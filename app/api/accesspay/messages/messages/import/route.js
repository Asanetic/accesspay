...
import { AddMessages } from "../MessagesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("messages", node, req, newRecordId, authData);

     const insertResult = await AddMessages(newRecordId, mutatedObj, req, authData);
