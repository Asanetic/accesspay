...
import { DeleteMessages } from '../messages/MessagesDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_messages_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'messages'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteMessages(deleteTokenDecode, whereStr);
